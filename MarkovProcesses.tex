...
therefore relax the kernel measurability to be universal rather than Borel.  Note also that the 
...
\item Generators of strongly continuous semigroups and Feller semigroups characterized
...
\subsection{Semigroups and Generators}
...
The set of essentially bounded functions on a measure space can be made into a Banach space under the essential supremum norm.  As is standard we have to pass to equivalence classes to do this so we first note when a function has an essential supremem of $0$.
...
\end{defn}
We reiterate that $A$ is not necessarily defined everywhere and that part of the definition of the generator is its domain of definition.  Unless it is explicity noted otherwise we will use $\mathcal{D}$ to denote the domain of a generator.

It is trivial to see the following.
\begin{prop}The generator is a linear operator on its domain of definition.
\end{prop}
\begin{proof}
This follows directly from linearity of the $T_t$ and linearity of limits.  If $v,w \in \mathcal{D}$ and $a \in \reals$ then
\begin{align*}
\lim_{t \to 0} \frac{T_t av - av}{t} &= a \lim_{t \to 0} \frac{T_t v - v}{t} = aAv \\
\lim_{t \to 0} \frac{T_t (v+w) - (v+w)}{t} &= \lim_{t \to 0} \frac{T_t v - v}{t} + \lim_{t \to 0} \frac{T_t 2 - w}{t}= Av + Aw \\
\end{align*}
\end{proof}

\subsection{Strongly Continuous and Feller Semigroups}
...
The Kolmogorov backward equation generalizes to arbitrary strongly continuous semigroups.
...
\begin{itemize}
\item[(i)] if $v \in X$ and $t \geq 0$ then $\int_0^t T_s v \, ds \in \mathcal{D}(A)$ and moreover $T_t v - v = A \int_0^t T_sv \, ds$
\item[(ii)] if $v \in \mathcal{D}(A)$ then $T_t v \in \mathcal{D}(A)$ for all $t \geq 0$ and moreover $\frac{d}{dt} T_t v = A T_t v = T_t A v$
\item[(iii)] for each $v \in X$ and $t \geq 0$ we have $T_t v - v = \int_0^t A T_sv \, ds = \int_0^t T_s A v \, ds$
\end{itemize}
\end{prop}
\begin{proof}
...
the continuity of $T_t$ we calculate for any $v \in \mathcal{D}(A)$ 
\begin{align*}
...
\end{align*}
To see that the first limit on the last line is zero we can use Lemma \ref{StronglyContinuousSemigroupNormBound} and the fact that $v \in \mathcal{D}(A)$ to see for all $0 < h \leq t$,
\begin{align*}
...
\begin{cor}Let $A$ be the generator of a strongly continuous semigroup on the Banach space $X$ then $A$ is a closed operator and $\mathcal{D}(A)$ is dense.
\end{cor}
\begin{proof}
If we let $v \in X$ then by Proposition \ref{StronglyContinuousSemigroupKolomgorovBackwardEquation} we know that $\int_0^t T_s v \, ds \in \mathcal{D}(A)$ for all $t \geq 0$ so it follows that $t^{-1} \int_0^t T_s v \, ds \in \mathcal{D}(A)$ for all $t > 0$.  Now observe that $\lim_{t \to 0} t^{-1} \int_0^t T_s v \, ds = v$ and we see that $\mathcal{D}(A)$ is dense.

Now suppose that $v_n \in \mathcal{D}(A)$ and suppose that $\lim_{n \to \infty} v_n = v$ and $\lim_{n \to \infty} A v_n  = w$.  Applying continuity of $T_t$ and Proposition \ref{StronglyContinuousSemigroupKolomgorovBackwardEquation}  we see that 
\begin{align*}
...
\end{align*}
which shows that $v \in \mathcal{D}(A)$ and $Av = w$.
\end{proof}

We now specialize the state space and assume that $S$ is a locally compact separable metric space.  

\subsection{Approximation of Feller Processes}