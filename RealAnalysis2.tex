...
\subsection{The space $D([0,\infty); S)$}
...
\begin{align*}
\lbrace (t_0, \dotsc, t_n) \mid t_0 < \dotsb < t_{n-1} < T \leq t_n \text{ and } t_i - t_{i-1} > \delta^\prime \rbrace 
&\subset \lbrace (t_0, \dotsc, t_n) \mid t_0 < \dotsb < t_{n-1} < T \leq t_n \text{ and } t_i - t_{i-1} > \delta \rbrace 
\end{align*}
...
$w^\prime(f,\delta, T)$ is non-decreasing in $T$, we know that $\lim_{t \to T^-} w^\prime(f,\delta, t)$ exists and $w^\prime(f,\delta, s) \leq \lim_{t \to T^-} w^\prime(f,\delta, t)$ for all $0 \leq s < T$.  For every $\eta < \rho$ we can pick a partition $t^\eta_0 < \dotsb < t^{\eta}_{m^\eta -1} < T - \eta \leq t^{\eta}_{m^\eta}$ with $\min_{1 \leq i \leq m^\eta} (t^\eta_i - t^\eta_{i-1}) > \delta$ and 
\begin{align*}
\max_{1 \leq i \leq m^\eta} \sup_{t^\eta_{i-1} \leq s < t < t^\eta_i} r(f(s),f(t)) &< w^\prime(f,\delta, T-\eta) + \epsilon/2 \leq \lim_{t \to T^-} w^\prime(f,\delta, t) + \epsilon/2
\end{align*}
If for some such partition we have $t^\eta_{m^\eta} \geq T$ then we can conclude $w(f,\delta,T) \leq \max_{1 \leq i \leq m^\eta} \sup_{t^\eta_{i-1} \leq s < t < t^\eta_i} r(f(s),f(t)) \leq \lim_{t \to T^-} w^\prime(f,\delta, t) + \epsilon/2$.  It that is not the case then
we modify these chosen partitions to make partitions with which we can bound $w(f,\delta,T)$.  Specifically for each $\eta < \rho$ for which $t^\eta_{m^\eta} < T$, we define $\tilde{t}^\eta_j = t^\eta_j$ for $0 \leq j < m^\eta$ and define $\tilde{t}^\eta_{m^\eta} = T$.  It is clear from the properties of the partition $\lbrace t^\eta_i \rbrace$ and the fact we have only moved the rightmost endpoint of the partition further to the right that $\tilde{t}^\eta_0 < \dotsb < \tilde{t}^\eta_{m^\eta -1} < T \leq \tilde{t}^\eta_{m^\eta}$, that
$\min_{1 \leq i \leq m^\eta} (\tilde{t}^\eta_i - \tilde{t}^\eta_{i-1}) \geq \min_{1 \leq i \leq m^\eta} (t^\eta_i - t^\eta_{i-1}) > \delta$ and for $1 \leq i < m^\eta$
\begin{align*}
\sup_{\tilde{t}^\eta_{i-1} \leq s < t < \tilde{t}^\eta_i} r(f(s),f(t))  &= \sup_{t^\eta_{i-1} \leq s < t < t^\eta_i} r(f(s),f(t))  \leq \sup_{t^\eta_{i-1} \leq s < t < t^\eta_i} r(f(s),f(t)) + \epsilon/2
\end{align*}
Moreover by the fact that $\eta < \rho$, we may pick a point $T^*$ such that  
\begin{align*}
(T - \rho) \vee t^\eta_{m^\eta-1} &< T^* < T - \eta \leq t^\eta_{m^\eta} < T
\end{align*} 
and therefore by breaking down the supremum into cases and using the triangle inequality 
\begin{align*}
&\sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < t < \tilde{t}^\eta_{m^\eta}} r(f(s),f(t)) \\
&=\sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < t \leq T^*} r(f(s),f(t)) \vee \sup_{T^* \leq s < t < \tilde{t}^\eta_{m^\eta}} r(f(s),f(t)) \vee \sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < T^* < t < \tilde{t}^\eta_{m^\eta}} r(f(s),f(t)) \\
&\leq \sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < t < \leq t^\eta_{m^\eta}} r(f(s),f(t)) \vee \sup_{T - \rho < s < t \leq T} r(f(s),f(t)) \vee \\
&\sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < T^* < t < \tilde{t}^\eta_{m^\eta}} \left[ r(f(s),f(T^*)) + r(f(T^*), f(t)) \right] \\
&\leq \sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < t < \leq t^\eta_{m^\eta}} r(f(s),f(t)) \vee \sup_{T - \rho < s < t \leq T} r(f(s),f(t)) \vee \\
&(\sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < t < \leq t^\eta_{m^\eta}} r(f(s),f(t)) + \sup_{T - \rho < s < t \leq T} r(f(s),f(t)) ) \\
&\leq \sup_{\tilde{t}^\eta_{m^\eta-1} \leq s < t < \leq t^\eta_{m^\eta}} r(f(s),f(t)) + \epsilon/2
\end{align*}
Thus we have 
\begin{align*}
w^\prime(f,\delta, T) \leq \max_{1 \leq i \leq m^\eta} \sup_{\tilde{t}^\eta_{i-1} \leq s < t < \tilde{t}^\eta_i} r(f(s),f(t))  &\leq 
\max_{1 \leq i \leq m^\eta} \sup_{t^\eta_{i-1} \leq s < t < t^\eta_i} r(f(s),f(t)) + \epsilon/2 \\
...
On the other hand it is clear from the fact that $w(f,\delta,T)$ is non-decreasing in $T$ that $\lim_{t \to T^-} w^\prime(f,\delta, t)  \leq w^\prime(f,\delta, T)$ and therefore $\lim_{t \to T^-} w^\prime(f,\delta, t)  = w^\prime(f,\delta, T)$.
\end{proof}

\begin{lem}\label{SkorohodInfiniteBorelGeneratedByProjections}For every $t \in [0,\infty)$ let $\pi_t : D([0,\infty); S) \to
  S$ be the evaluation map $\pi_t(f) = f(t)$.  The Borel $\sigma$-algebra on $D([0,\infty); S)$ is
  equal to $\sigma(\lbrace \pi_t \mid t \in [0,\infty) \rbrace)$ and
...
TODO: Understand the Either and Kurtz construction and its relationship with Lindvall's.  One proposal is that the only difference is the manner in which we are embedding $D([0,\infty))$ in $D_0^\infty$ (accounting for the lack of linearity).  Here we define for each $n \in \naturals$ the map $c_n : D([0,\infty)) \to D([0,\infty])$ by $c_n(f)(t) = f(t \wedge n)$ and then map $f$ to $\Psi(f) = (c_1(f), c_2(f), \dotsc)$. The key things that we need to have be true are that
\begin{itemize}
